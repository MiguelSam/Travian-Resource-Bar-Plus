...
// @description    Shows travian resources (updated for T4.4 and Ancient Europe)
...
var namespace = 'https://greasyfork.org/scripts/5923-travian-resource-bar-plus';
...
DICT = {
	en: {
		...
		ancientEu : "Ancient Europe server",
		ancientEuh : "Check if server is Ancient Europe (Tournament Square - double speed bonus)",
	...
	ua: { fb : "ru",
		...
		sspeedh : "0 - auto, 1 (1x), 2 (2x), 3 (3x або швидкісний), ...",
		ancientC : "AC сервер",
		...
		dontwrap : "Не давати звужувати сторінку (T3.6)",
		dontwraph : 'якщо список поселень стибає донизу або перекошується',
 ...
 ar: { // Arabic, thx ww_start_t
	...
	bg: { // Bulgarian language, thx Dushevadeca
		...
		searchlast : "Намери последният от", 
		...
		dontwrap : "Не позволявай припокриването на страницата", 
		dontwraph : 'ако списъка със селата отскача надолу', 
	...
	ro: { // Romanian language, thx bubulu
		...
		sspeedh : "0 - auto, 1 (1x), 2 (2x), 3 (3x aka speed), ... etc.",
		ancientEu : "Server Europa Antică",
		ancientEuh : "Bifați dacă server-ul este Europa Antică (Arenă - bonus de viteză dublu)",
		...
		dontwrap : "Nu înfășura paginile",
		dontwraph : 'dacă lista de sate sare în jos',
		...
		bidinc : "Mărește oferta cu (în arginți)",
		bidinch : 'pentru licitații',
...
function getTTime(dist, speed, arena, artefact, shoes) {
	...
	var abonus = (RB.Setup[46] == 0 ? 0.1 : 0.2); //if AncientEU then double the Tournament Square bonus
...
function rbSetup () {
	...
	var aRBS = [
		['I', 0, gtext("info")],
			...
			['CB', 46, gtext("ancientEu"), gtext("ancientEuh")],
			...
			if( i > 1 ) {
				...
				if( Math.abs(minLeft) < 600 ) {
					setInterval(function(x){return function(){
						(x.style.color=='white') ? (x.style.color='black') : (x.style.color='white');}
					}(timerOv[t].obj),1000);
				}
...
function karteDistance4 () {
	...
	if( vilView ) {
		if( tileDFL ) {
			...
			if( ! ver42FL ) addFarmToMapT4( vilView );
		}
		tileDFL = false;
	} else {
		tileDFL = true;
...
function show_alert () {
	...
	switch (parseInt(RB.Setup[28])) {
		...
		case 3: // EMBED audio
				...
				break;
		case 4: // FLASH player
				cont.appendChild($e('EMBED',[['id',allIDs[22]],['type','application/x-shockwave-flash'],['flashvars','mp3='+RB.Setup[29]+'&autoplay=1'],['src','http://flash-mp3-player.net/medias/player_mp3_mini.swf'],['width',200],['height',20],['bgcolor','#085c68']]));
				break;
	}
	lastAlert = nt;
}
...
//	getAjaxToken();
	...
	function cropFindReq () {
		...
		cont.appendChild($ee('DIV',$ee('span','searching...',[['style','color:black;'],['id',allIDs[0]]]),[['id',allIDs[18]]]));
		ss = $g(allIDs[0],cont);
		setInterval(function(x){return function(){
			(x.style.color=='white') ? (x.style.color='black') : (x.style.color='white');}
		}(ss),500);
					...
					if( ! iaFL ) {
						...
						if( anim.snapshotLength > 0 ) {
							for( var i=0; i<anim.snapshotLength; i++ ) {
								...
								if ( tt == 38 ) { //add crocs
									x[3].rows[x[2]].cells[2].appendChild($ee('SPAN',tc+"x"));
									x[3].rows[x[2]].cells[2].appendChild(trImg('unit u38'));
								}
								if ( tt == 40 ) { //add elephants
									x[3].rows[x[2]].cells[2].appendChild($ee('SPAN',tc+"x"));
									x[3].rows[x[2]].cells[2].appendChild(trImg('unit u40'));
								}
							}
						}
					}
	...
	var anDiv = ver4FL ? $em('SPAN',[' ',trImg('unit u40'),':',anim]): "";
...
function TM_ShowMainBuildingNumbers(){
	...
	if ( mapInfo ) {
		...
		if ( mapInfo ) {
			countArray = 18;
			dorf = 1;
			if( ver4FL ) var levels = $xf('./DIV','l',$g('village_map'));
	...
	for (var i = 0; i < countArray; i++) {
		...
		if (dorf == 1){
			...
			smallDIV = ver4FL ? levels.snapshotItem(i): $g('rx').appendChild($ee('div',BuildingLevel[0],[['class',allIDs[42]]]));
			doubleFL = $gt('DIV',smallDIV).length>0 ? true:false;
		}
...
// start script
	...
	if( /position_details.php\?(.*&)?[zdxy]=/.test(crtPath) ) { troopsOasis(); distanceToMyVillages(); viewMessageIWK(); linkOnT4Karte(); addToMapT4(); if( ! ver42FL ) addFarmToMapT4(); }
